...
Процедура ВыгрузитьКонфигурациюВФайлы(Знач КаталогВыгрузки, Знач ФорматВыгрузки = "", Знач ТолькоИзмененные = Ложь, Знач ПутьКФайлуВерсийДляСравнения = "", Знач ИмяРасширения = "") Экспорт
...
//  НомерВерсии  		  - Число -  Номер версии в истории в хранилище конфигурации, если 0 или меньше, то берется последняя версия.
//
Процедура ЗагрузитьКонфигурациюИзХранилища(Знач СтрокаСоединения, Знач ПользовательХранилища, Знач ПарольХранилища = "", Знач НомерВерсииХранилища = 0, ИмяРасширения = "") Экспорт
...
//  НомерВерсии  		  - Число -  Номер версии в истории в хранилище конфигурации, если 0 или меньше, то берется последняя версия.
//
Процедура ОбновитьКонфигурациюБазыДанныхИзХранилища(Знач СтрокаСоединения, Знач ПользовательХранилища, Знач ПарольХранилища = "", Знач НомерВерсииХранилища = 0, ИмяРасширения = "") Экспорт
...
#Область ДополнительныеФункцииСравненияХранилища

Процедура ПолучитьОтчетОСравненииКонфигурацииСХранилищем(Знач СтрокаСоединения, 
	Знач ПользовательХранилища,
	Знач ПарольХранилища = "",
	Знач ВерсияХранилища = "-1",
	Знач ПутьКОтчету,
	Знач ТипОтчета = Неопределено,
	Знач ФорматОтчета = "txt",
	Знач ИмяРасширения = "") Экспорт
	...
	ПараметрыЗапуска = СтандартныеПараметрыЗапускаКонфигуратора();
	ПараметрыЗапуска.Добавить("/ConfigurationRepositoryF """+СтрокаСоединения + """");
	...
	Если ЗначениеЗаполнено(ИмяРасширения) Тогда
		...
		ПараметрыЗапуска.Добавить("-SecondConfigurationType ExtensionConfigurationRepository");
		ПараметрыЗапуска.Добавить(СтрШаблон("-SecondName ""%1""", ИмяРасширения));

	Иначе

		ПараметрыЗапуска.Добавить("-SecondConfigurationType ConfigurationRepository");

	КонецЕсли;

	ПараметрыЗапуска.Добавить(СтрШаблон("-SecondVersion %1", ВерсияХранилища));
...
Функция КонфигурацияИХранилищеИдентичны(Знач СтрокаСоединения, 
	Знач ПользовательХранилища,
	Знач ПарольХранилища = "",
	Знач ИмяРасширения = "") Экспорт
	...
	ЭталонОтчетаБезИзменений = НСтр(
		...
		|***- Объект изменен
		|-->- Объект присутствует только в %1 конфигурации
		|<--- Объект присутствует только в %2 хранилища
		|^- Порядок объекта изменен'
		|;
		|en = '
		|***- Object changed
	...
	Если ЗначениеЗаполнено(ИмяРасширения) Тогда
		...
		СтрокаДополненияРасширении = НСтр("ru = 'расширении конфигурации'; en = 'configuration extension'", 
		мКонтекстКоманды.КодЯзыкаСеанса);
		
	Иначе

		СтрокаДополненияРасширении = НСтр("ru = 'конфигурации'; en = 'configuration extension'", 
		мКонтекстКоманды.КодЯзыкаСеанса);	
	...
	ПолучитьОтчетОСравненииКонфигурацииСХранилищем(СтрокаСоединения,
...
Процедура ВыгрузитьКонфигурациюХранилищаВФайл(Знач СтрокаСоединения, 
	Знач ПользовательХранилища,
	Знач ПарольХранилища = "",
	Знач ВерсияХранилища = "-1",
	Знач ФайлКонфигурации) Экспорт
	...
	Параметры.Добавить("/ConfigurationRepositoryDumpCfg """ + ФайлКонфигурации + """" + СтрШаблон(" -v %1", ВерсияХранилища));

	ВыполнитьКоманду(Параметры);
	
КонецПроцедуры

Процедура СократитьЖурналРегистрации(Знач НоваяГраница, 
	Знач ИмяФайлаКопии = "",
	Знач СохранитьСРазделением = Ложь) Экспорт
		
	ПараметрыЗапуска = СтандартныеПараметрыЗапускаКонфигуратора();
	ПараметрыЗапуска.Добавить("/ReduceEventLogSize");
	ПараметрыЗапуска.Добавить(НоваяГраница);
	
	Если ЗначениеЗаполнено(ИмяФайлаКопии) Тогда

		ПараметрыЗапуска.Добавить("-saveAs");
		ПараметрыЗапуска.Добавить(ОбернутьВКавычки(ИмяФайлаКопии));

	КонецЕсли;

	Если СохранитьСРазделением Тогда
		
		ПараметрыЗапуска.Добавить("-KeepSplitting");
